...
if __name__ == '__main__':
	...
	# 添加课程
	c = Course(name=u'初级提高班', gtype=GameType.bridge, tid=1, time='3月5日起连续15周，每周2小时', start='2016 03-05', 
		end='2016 12-01', count=15, period=120, charge=3600, max_student=16, min_student=12, audition=1, discount=1, 
	...
	for i in range(1, len(time)):
		schedule = CourseSchedule(cid=1, rid=1, time=time[i], mteacher='1')
		db.session.add(schedule)
	db.session.commit()

	c = Course(name=u'桥牌高级班', gtype=GameType.bridge, tid=1, time='3月5日起连续15周，每周2小时', start='2016 03-05', 
		end='2016 12-01', count=15, period=120, charge=4500, max_student=20, min_student=12, audition=1, discount=1, 
	...
	for i in range(1, len(time) + 1):
		schedule = CourseSchedule(cid=2, rid=1, time=time[i - 1], mteacher='1')
		db.session.add(schedule)
	db.session.commit()
