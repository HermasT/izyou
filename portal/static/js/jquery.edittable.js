    ...
    $.fn.editTable = function (options) {
        ...
        function buildRow(data, len) {
            ...
            if (s.has_add_icon) {
                ...
                return $('<tr/>', {
                    html: rowcontent + '<td><a class="editrow icon-button" href="#">E</a> <a class="delrow icon-button" href="#">-</a></td>'
                });
            }  
        }
        ...
        $table.on('click', '.editrow', function () {
            ...
            if (s.edit_callback) {
                ...
                var row_data = exportData()[row_index];
                s.edit_callback(row_index, row_data, s.protocol);
            }
            return false;
        });
        ...
        $table.on('click', '.delrow', function () {
            ...
            if (s.delete_callback) {
                var row_index = $(this).closest('tr').index();
                var row_data = exportData()[row_index];
                s.delete_callback(row_index, row_data, s.protocol);
            } else {
                rownumber -= 1;
                checkButtons();
